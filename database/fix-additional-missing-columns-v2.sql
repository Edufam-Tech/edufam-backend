-- ====================================
-- FIX ADDITIONAL MISSING COLUMNS (VERSION 2)
-- ====================================
-- This file fixes additional database schema issues causing API errors
-- Version 2: Handles potential conflicts with existing objects

-- First, let's check if there are any problematic objects and clean them up
DO $$ 
BEGIN
    -- Drop any views that might reference problematic columns
    DROP VIEW IF EXISTS performance_appraisals_view CASCADE;
    DROP VIEW IF EXISTS appraisal_summary_view CASCADE;
    DROP VIEW IF EXISTS employee_performance_view CASCADE;
    
    -- Drop any functions that might reference problematic columns
    DROP FUNCTION IF EXISTS get_performance_summary(UUID) CASCADE;
    DROP FUNCTION IF EXISTS calculate_appraisal_score(UUID) CASCADE;
    
    -- Drop any triggers that might reference problematic columns
    DROP TRIGGER IF EXISTS update_performance_timestamp ON performance_appraisals CASCADE;
    DROP TRIGGER IF EXISTS update_appraisal_cycle_timestamp ON appraisal_cycles CASCADE;
...
CREATE TABLE IF NOT EXISTS appraisal_cycles (
    ...
    -- Cycle Details
    name VARCHAR(100) NOT NULL,
    cycle_year INTEGER NOT NULL, -- Changed from 'year' to 'cycle_year' to avoid conflicts
    description TEXT,
    ...
    -- Constraints
    CONSTRAINT unique_school_cycle_year UNIQUE (school_id, cycle_year)
);
...
CREATE INDEX IF NOT EXISTS idx_appraisal_cycles_year ON appraisal_cycles(cycle_year);
